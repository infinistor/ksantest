...
func TestEncryptionSseCMethodHead(t *testing.T) {
	...
	if _, err := s.client.PutObject(context.Background(), sseCPutInput(bucket, "obj", randomTextToLong(1000))); err != nil {
		t.Fatal(err)
	}
...
func TestEncryptionSseCPresent(t *testing.T) {
	t.Parallel()
	s, bucket, _ := newSSECObject(t, 1000, 6)
...
func TestEncryptionSseCOtherKey(t *testing.T) {
	t.Parallel()
	s, bucket, _ := newSSECObject(t, 100, 7)
...
func TestEncryptionSseCInvalidMd5(t *testing.T) {
	...
	input := sseCPutInput(bucket, "obj", randomTextToLong(100))
...
func TestEncryptionSseCNoKey(t *testing.T) {
	...
	_, err := s.client.PutObject(context.Background(), &s3.PutObjectInput{Bucket: aws.String(bucket), Key: aws.String("obj"), Body: bytes.NewReader(randomTextToLong(100)), SSECustomerKeyMD5: aws.String(sseCKeyMD5)})
	assertHTTPError(t, err, 400)
}
...
func TestEncryptionKeyNoSseC(t *testing.T) {
	...
	_, err := s.client.PutObject(context.Background(), &s3.PutObjectInput{Bucket: aws.String(bucket), Key: aws.String("obj"), Body: bytes.NewReader(randomTextToLong(100)), SSECustomerKey: aws.String(sseCKey), SSECustomerKeyMD5: aws.String(sseCKeyMD5)})
	assertHTTPError(t, err, 400)
}
...
func TestEncryptionSseCGetObjectMany(t *testing.T) {
	t.Parallel()
	s, bucket, body := newSSECObject(t, 15*1024*1024, 15)
	for i := 0; i < 50; i++ {
...
func TestEncryptionSseCRangeObjectMany(t *testing.T) {
	t.Parallel()
	s, bucket, body := newSSECObject(t, 15*1024*1024, 16)
	assertSSECObject(t, s.client, bucket, "obj", body)
	for i := 0; i < 50; i++ {
...
func TestEncryptionSseCPutObjectOverwriteMultipartUpload(t *testing.T) {
	...
	completeSSECMultipart(t, s.client, bucket, key, randomTextToLong(10*1024*1024), nil)
...
func unblockSseC(t *testing.T, s *suite, bucket string) {
	...
	req, err := http.NewRequest(http.MethodPut, "https://"+host+"/?encryption", bytes.NewReader(body))
	if err != nil {
		t.Fatalf("unblock SSE-C request: %v", err)
	}
	...
	credVal, err := credentials.NewStaticCredentialsProvider(s.cfg.Main.AccessKey, s.cfg.Main.SecretKey, "").Retrieve(context.Background())
	if err != nil {
		t.Fatalf("unblock SSE-C credentials: %v", err)
	}
	if err := v4.NewSigner().SignHTTP(context.Background(), credVal, req, payloadHash, "s3", region, time.Now().UTC()); err != nil {
		t.Fatalf("unblock SSE-C sign: %v", err)
	}
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatalf("unblock SSE-C PutBucketEncryption: %v", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode/100 != 2 {
		msg, _ := io.ReadAll(resp.Body)
		t.Fatalf("unblock SSE-C PutBucketEncryption: status=%d body=%s", resp.StatusCode, msg)
	}

	for i := 0; i < 5; i++ {
		_, putErr := s.client.PutObject(context.Background(), sseCPutInput(bucket, ".ssec-unblock-probe", []byte("x")))
		if putErr == nil {
			_, _ = s.client.DeleteObject(context.Background(), &s3.DeleteObjectInput{Bucket: aws.String(bucket), Key: aws.String(".ssec-unblock-probe")})
			return
		}
		if !strings.Contains(putErr.Error(), "Customer provided keys") && !strings.Contains(putErr.Error(), "SSE-C") {
			return
		}
		time.Sleep(time.Second)
	}
	t.Fatalf("SSE-C unblock failed: %s", bucket)
}
...
func newSSECObject(t *testing.T, size int, id ...int) (*suite, string, []byte) {
	t.Helper()
	s := newSuite(t)
	bucket := ssecBucket(t, s, id...)
	body := randomTextToLong(size)
	if _, err := s.client.PutObject(context.Background(), sseCPutInput(bucket, "obj", body)); err != nil {
		t.Fatal(err)
	}
	return s, bucket, body
}
