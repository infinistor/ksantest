...
기본적으로 최대 4개 테스트를 병렬 실행합니다 (`go test -parallel`). Linux/macOS에서는 `GO_TEST_PARALLEL`로 동시성을 변경합니다.
...
Python 및 Java 실행 스크립트와 동일하게 설정, 테스트 클래스, 테스트 메서드 순서로 지정합니다. 설정은 `.ini` 없이 기본 이름만 넘기고, 메서드는 Java camelCase와 Python snake_case를 모두 허용합니다.
...
```bash
./start-function.sh config PutBucket testBucketCreateNamingBadIp
...
```text
go/
├── *_test.go                # 클래스별 Go 테스트 (시나리오=Test 함수)
