...
namespace s3tests2
{
    ...
    {
        [TestMethod("test_set_cors")]
        [TestProperty(MainData.Major, "Cors")]
        [TestProperty(MainData.Minor, "Check")]
        [TestProperty(MainData.Explanation, "버킷의 cors정보 세팅 성공 확인")]
        ...
        public void test_set_cors()
        {
            ...
            Assert.IsNull(Response.Configuration);

            Client.PutCORSConfiguration(BucketName, CORSConfig);
            Response = Client.GetCORSConfiguration(BucketName);
            CollectionAssert.AreEqual(AllowedMethods, Response.Configuration.Rules[0].AllowedMethods);
            CollectionAssert.AreEqual(AllowedOrigins, Response.Configuration.Rules[0].AllowedOrigins);
        ...
        [TestMethod("test_cors_origin_response")]
        [TestProperty(MainData.Major, "Cors")]
        [TestProperty(MainData.Minor, "Post")]
        [TestProperty(MainData.Explanation, "버킷의 cors정보를 URL로 읽고 쓰기 성공/실패 확인")]
        [TestProperty(MainData.Result, MainData.ResultSuccess)]
        ...
        [TestMethod("test_cors_origin_wildcard")]
        [TestProperty(MainData.Major, "Cors")]
        [TestProperty(MainData.Minor, "Post")]
        [TestProperty(MainData.Explanation, "와일드카드 문자만 입력하여 cors설정을 하였을때 정상적으로 동작하는지 확인")]
        [TestProperty(MainData.Result, MainData.ResultSuccess)]
        ...
        [TestMethod("test_cors_header_option")]
        [TestProperty(MainData.Major, "Cors")]
        [TestProperty(MainData.Minor, "Post")]
        [TestProperty(MainData.Explanation, "cors옵션에서 사용자 추가 헤더를 설정하고 존재하지 않는 헤더를 request 설정한 채로 curs호출하면 실패하는지 확인")]
        [TestProperty(MainData.Result, MainData.ResultSuccess)]
