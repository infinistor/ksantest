...
namespace s3tests2
{
	...
	{
		[TestMethod("test_buckets_create_then_list")]
		[TestProperty(MainData.Major, "ListBuckets")]
		[TestProperty(MainData.Minor, "Get")]
		[TestProperty(MainData.Explanation, "여러개의 버킷 생성해서 목록 조회 확인")]
		...
		[TestMethod("test_list_buckets_invalid_auth")]
		[TestProperty(MainData.Major, "ListBuckets")]
		[TestProperty(MainData.Minor, "ERROR")]
		[TestProperty(MainData.Explanation, "존재하지 않는 사용자가 버킷목록 조회시 에러 확인")]
		...
		public void test_list_buckets_invalid_auth()
		{
			...
			var e = Assert.ThrowsException<AggregateException>(() => BadAuthClient.ListBuckets());
			Assert.AreEqual(HttpStatusCode.Forbidden, GetStatus(e));
			Assert.AreEqual(MainData.InvalidAccessKeyId, GetErrorCode(e));
		}

		[TestMethod("test_list_buckets_bad_auth")]
		[TestProperty(MainData.Major, "ListBuckets")]
		[TestProperty(MainData.Minor, "ERROR")]
		[TestProperty(MainData.Explanation, "로그인정보를 잘못입력한 사용자가 버킷목록 조회시 에러 확인")]
		[TestProperty(MainData.Result, MainData.ResultFailure)]
		public void test_list_buckets_bad_auth()
		{
			...
			var e = Assert.ThrowsException<AggregateException>(() => BadAuthClient.ListBuckets());
			Assert.AreEqual(HttpStatusCode.Forbidden, GetStatus(e));
			Assert.AreEqual(MainData.SignatureDoesNotMatch, GetErrorCode(e));
		}

		[TestMethod("test_head_bucket")]
		[Ignore("netcore에서는 HeadBucket API가 존재하지 않음")]
		[TestProperty(MainData.Major, "ListBuckets")]
		[TestProperty(MainData.Minor, "Metadata")]
		[TestProperty(MainData.Explanation, "버킷의 메타데이터를 가져올 수 있는지 확인")]
		[TestProperty(MainData.Result, MainData.ResultFailure)]
