...
namespace s3tests
{
	public class Replication : TestBase
	{
		...
		[Fact(DisplayName = "test_replication_no_rule")]
		[Trait(MainData.Major, "Replication")]
		[Trait(MainData.Minor, "Check")]
		[Trait(MainData.Explanation, "복제설정중 role이 없어도 설정되는지 확인")]
		[Trait(MainData.Result, MainData.ResultSuccess)]
		...
		[Fact(DisplayName = "test_replication_full_copy")]
		[Trait(MainData.Major, "Replication")]
		[Trait(MainData.Minor, "Check")]
		[Trait(MainData.Explanation, "버킷의 복제설정이 올바르게 동작하는지 확인")]
		[Trait(MainData.Result, MainData.ResultSuccess)]
		public void test_replication_full_copy()
		{
			...
			Assert.Equal(SourceKeys, TargetKeys);
		...
		[Fact(DisplayName = "test_replication_prefix_copy")]
		[Trait(MainData.Major, "Replication")]
		[Trait(MainData.Minor, "Check")]
		[Trait(MainData.Explanation, "버킷의 복제 설정중 prefix가 올바르게 동작하는지 확인")]
		[Trait(MainData.Result, MainData.ResultSuccess)]
		...
		[Fact(DisplayName = "test_replication_deletemarker_copy")]
		[Trait(MainData.Major, "Replication")]
		[Trait(MainData.Minor, "Check")]
		[Trait(MainData.Explanation, "버킷의 복제 설정중 DeleteMarker가 올바르게 동작하는지 확인")]
		[Trait(MainData.Result, MainData.ResultSuccess)]
		public void test_replication_deletemarker_copy()
		{
			...
			Assert.Equal(SourceKeys.Count, Target2Keys.Count);
			Assert.Equal(Keys, Target1Keys);
